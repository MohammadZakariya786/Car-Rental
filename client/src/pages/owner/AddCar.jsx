...
const AddCar = () => {
    ...
    const {axios,currency,fetchCars,navigate}=useAppContext()
    ...
    const onSubmitHandler = async(e)=>{
        ...
        try {
            ...
            // convert comma-separated features text into array
            const featuresArray = featuresText.split(',').map(f=>f.trim()).filter(Boolean)
            const carDataToSend = {...car, features: featuresArray}
    ...
    <div className='px-4 py-10 md:px-10 flex-1'>
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col gap-5 text-gray-500 text-sm mt-6 max-w-xl'>
            ...
            <div className='flex items-center gap-2 w-full'>
                ...
                <p className='text-gray-500 text-sm'>Upload a picture of your card</p>
            </div>
            ...
            <div className='flex flex-col w-full'>
                ...
                <textarea rows={5} placeholder='e.g. A luxorious SUV with a spacious interior and a powerful engine.' required className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none'
                value={car.description} onChange={e=>setCar({...car, description:e.target.value})}></textarea>
            </div>

            {/* features (comma separated) */}
            <div className='flex flex-col w-full'>
                <label>Features <span className='text-xs text-gray-400'>(comma separated)</span></label>
                <input type='text' placeholder='e.g. Bluetooth, GPS, Heated Seats' className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none'
